...
Hyperliquid DEX 自动跟单交易机器人。通过 WebSocket 实时监控目标钱包，自动复制其永续合约交易，按比例智能调整仓位大小，支持模拟交易和 Telegram 远程控制。
...
编辑 `.env`，至少填写 `TARGET_WALLET_ADDRESS`（跟单目标地址）。首次使用建议保持模拟模式。
...
| `TARGET_WALLET_ADDRESS` | string | **必填** | 跟单目标地址（钱包或金库地址） |
...
启动 main.py
    ...
    ├── 4. 获取目标钱包和自身账户的初始状态
    ├── 5. 计算钱包余额比例（如 1:10）
    ├── 6. [可选] 复制目标已有持仓（COPY_OPEN_POSITIONS）
    ├── 7. [可选] 复制目标已有挂单（COPY_EXISTING_ORDERS）
...
| 命令 | 功能 | 示例输出 |
|------|------|---------|
| `/status` | 机器人运行状态、余额、模式 | 运行中 · 模拟模式 · 余额 $10,234.56 · 已跟单 15 笔 |
| `/positions` | 当前所有持仓及盈亏 | BTC 做多 0.02 @ $60,000 · 盈亏 +$45.20 (+3.8%) |
