...
- **全事件覆盖** — 支持新开仓、加仓、翻转仓位（如空转多）等开仓类操作的跟单
- **智能过滤** — 平仓/减仓操作不跟单成交事件（避免你没有对应持仓时出错），平仓通过持仓关闭事件单独处理
...
- **资产特定上限** — 不同资产自动限制最大杠杆
  ...
  | ATOM / LTC / BCH / LINK / UNI / APE / APT / SUI 等 | 10x |
  | HYPE / TRUMP / MELANIA / PUMP / PEPE / BONK / WIF 等 | 10x |
  | 其他未列出资产 | 10x（默认） |
- **整数取整** — 自动适配 Hyperliquid 仅支持整数杠杆的要求

### 订单类型
- **市价单** — 即时执行（默认）
- **限价单** — 可将市价单转换为以成交价挂限价单（`USE_LIMIT_ORDERS=true`）
...
│   ├── hyperliquid/
│   │   ├── models.py             # 数据模型（Position, Order, UserState, WebSocketUpdate）
...
| `LEVERAGE_ADJUSTMENT` | float | `1.0`* | 杠杆倍率调整（0.5 = 保守，1.0 = 完全匹配，2.0 = 激进） |

> *`LEVERAGE_ADJUSTMENT` 在 `.env.example` 中默认 `1.0`（完全匹配目标杠杆）。若未设置该环境变量，代码内部回退默认值为 `0.5`。
...
> **注意：** `.env.example` 中的 `DATABASE_URL` 为预留配置，当前版本代码未使用。
...
WebSocket 接收事件
    │
    ├── 订单成交（Fill）
    │   ├── Open/Add（开仓/加仓）──→ 获取持仓 → 计算仓位 → 检查$10最小值 → 调整杠杆 → 执行 → 通知
    │   ├── 翻转（Short>Long 等）──→ 延迟重试获取持仓 → 同上流程
    │   └── Close/Reduce（平仓/减仓）──→ 跳过（由持仓关闭事件处理）
    │
    ├── 持仓变更（Position）
    │   ├── 新持仓 ──→ 记录（交易执行由成交事件处理）
    │   ├── 持仓归零 ──→ 平掉你的对应仓位 → 计算盈亏 → 更新余额 → 通知
    │   └── 数量变化 ──→ 触发持仓更新回调
    │
    ├── 新挂单（Order）──→ 计算你的订单大小 → 执行对应限价单 → 通知
    │
    └── 订阅确认 ──→ 记录日志
```
...
| `/resume` | 恢复跟单 | 跟单已恢复 |
| `/stop` | 停止机器人（弹出确认对话框，可选平仓/保留持仓/取消） | 确认界面 → 机器人已停止 |
...
| **限价单模式** | 可选将市价单转为限价单执行，避免滑点 |
| **最大持仓数限制** | 可配置同时持仓上限 |
| **最大权益限制** | 账户权益超过阈值后自动停止跟单 |
| **资产级杠杆上限** | 按资产类别设置最大杠杆（BTC/ETH 50x, SOL 等 20x, 其他 10x），覆盖 30+ 资产 |
| **资产黑名单** | 屏蔽特定高风险资产 |
| **Telegram 鉴权** | 仅响应指定 Chat ID 的命令 |
| **优雅退出** | Ctrl+C 信号处理，确保资源正确释放 |
| **平仓/减仓过滤** | 成交事件中的 Close/Reduce 方向自动跳过，避免无持仓时误操作 |
| **状态刷新延迟** | 成交处理前等待 1.5s 确保状态同步 |
| **仓位翻转检测** | 检测 "Short > Long" 等翻转操作，延迟重试获取最新持仓后跟单 |
...
以下情况机器人会跳过跟单：
- 成交方向为 Close（平仓）或 Reduce（减仓）— 平仓由持仓关闭事件单独处理
...
- 仓位翻转或开仓后重试仍未找到持仓数据（交易所延迟或已即时平仓）
...
**平仓**会跟。当目标的持仓归零时，机器人通过 WebSocket 的持仓关闭事件检测到变化，会自动平掉你对应的持仓并计算盈亏。

**减仓**目前不跟。成交事件中的 "Close" 和 "Reduce" 方向会被跳过，因为你可能没有对应的持仓可以减。如果目标完全平仓（持仓归零），则会触发平仓逻辑。

### Q: 什么是仓位翻转？

仓位翻转是指目标从多头直接切换到空头（或反过来），例如 "Short > Long"。机器人会检测这类翻转事件，在延迟 1.5 秒等待状态同步后获取新持仓数据，然后按比例跟单新方向的仓位。
